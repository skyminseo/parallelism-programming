...
def select_folder_dialog():
    ...
    folder_path = f"(current_path)/save_2/"
    return folder_path
...
def detect_faces(video_file, face_crop_folder, result, process_id):
    ...
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.7) as face_detection:
        ...
        while True:
            ...
            if results.detections:
                for detection in results.detections:
                    ...
                    if not is_duplicate:
                        ...
                        cv2.moveWindow(
                            f'face_crop_{process_id}_{num_detection}', 50 + process_id * 150, 10 + num_detection * 60)
            ...
            frame_width = int(frame.shape[1] * 0.3)
            frame_height = int(frame.shape[0] * 0.3)

            cv2.imshow(f'video_0', cv2.resize(
                frame, (frame_width, frame_height)))
            cv2.moveWindow(f'video_0', 500, 10)

            cv2.imshow(f'video_1', cv2.resize(
                frame, (frame_width, frame_height)))
            cv2.moveWindow(f'video_1', 500, frame_height + 55)

            cv2.imshow(f'video_2', cv2.resize(
                frame, (frame_width, frame_height)))
            cv2.moveWindow(f'video_2', 500, 2 * (frame_height + 50))
...
if __name__ == '__main__':
    ...
    video_files = ['audience1.mp4', 'audience2.mp4', 'audience3.mp4']
